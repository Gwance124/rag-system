...
- **Current lab setup:** `llama-nv-embed-reason-3b` is served by vLLM on `solab-g3:8000` (OpenAI-compatible `/v1/embeddings`). The benchmark and corpus datasets are on `solab-p7`, which is the benchmark execution host. Hugging Face loading at benchmark time is cache-only (`local_files_only=True` plus offline mode); a cache miss must fail rather than reach the internet.
